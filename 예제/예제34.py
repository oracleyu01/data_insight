##▣ 예제34.  다변량 데이터 시각화 (Pair Plot, Joint Plot)
...
## 1.5 지도 시각화에서 마커 및 클러스터 활용
지도 시각화에서는 여러 개의 마커를 효과적으로 표현하기 위해 `folium`의 `Marker`와 `MarkerCluster`를 활용할 수 있습니다.
...
### 마커 클러스터 활용
여러 개의 마커가 겹쳐 보이는 문제를 해결하기 위해 `MarkerCluster`를 사용할 수 있습니다.
```python
...
# 여러 개의 마커 추가
locations = [[37.5665, 126.9780], [37.5796, 126.9770], [37.5700, 126.9900], [37.5610, 126.9950]]
labels = ["서울", "경복궁", "종로", "명동"]

for loc, label in zip(locations, labels):
    folium.Marker(loc, popup=label).add_to(marker_cluster)

# 지도 출력
map
```

## 1.6 지도 시각화에서 마커 및 클러스터 활용의 예시
- 매장 위치 및 지점 관리
- 실시간 사고 및 교통 데이터 시각화
- 물류 및 배달 경로 최적화

## 1.7 객관식 문제
다음 중 Folium에서 다중 마커를 클러스터링하는 데 사용되는 기능은 무엇인가?
